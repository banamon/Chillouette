...
public class UniGifImage : MonoBehaviour
{
    ...
    {
        Debug.Log("Unigifの中のDebug");
        if (string.IsNullOrEmpty(url))
        {
            //Debug.LogError("URL is nothing.");
            //↓
            Debug.LogError("URL is nothing.:"+url);
            yield break;
        }
        ...
        Debug.Log("wwwでファイルにアクセス");//Debug田賀
        // Load file
        using (WWW www = new WWW(path))
        {
            yield return www;

            Debug.Log("wwwでファイルにアクセス完了");//Debug田賀
